function [input_im_,input_im,w]=removeframe(imname)

threshold=0.5;
input_im_ = imread(imname);
input_im=im2double(input_im_);
...
for k=1:33 % we assume that the frame is not wider than 30 pixels.
    pbt=mean(edgemap(k,:));
...
if flagrm>1 
    ...
    input_im_ = input_im_(t:m-d+1,l:n-r+1,:);
